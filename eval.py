...
class Eval:
    ...
    def report_speed(self, model, batch, times=100):
        ...
        for _ in range(times):
            pred, dilate = model.forward(data, training=False)
        time_cost2 = (time.time() - start) / times
        for _ in range(times):
            output = self.structure.representer.represent(batch, pred, dilate, is_output_polygon=self.args['polygon'])
        time_cost = (time.time() - start) / times
        total_fps.append(1 / time_cost)
        self.logger.info('Params: %s, Inference speed: %fms, FPS: %f, %f  Forward: %f %f' % (
            str(sum(p.numel() for p in model.parameters() if p.requires_grad)),
            time_cost * 1000, 1 / time_cost, sum(total_fps) / len(total_fps), time_cost2 * 1000, time_cost2 * 100 / time_cost))
            ...
            if self.args['polygon']:
                with open(result_file_path, 'wt') as res:
                    for i, box in enumerate(boxes):
                        ...
                        det_x = box[:, 0]
                        det_y = box[:, 1]
                        box = np.stack((det_x, det_y), axis=1).reshape(-1).tolist()
                        result = ",".join([str(int(x)) for x in box])
                        res.write(result + ',' + str(scores[i]) + '\n')
            else:
                with open(result_file_path, 'wt') as res:
                    for i, box in enumerate(boxes):
                        box = np.array(box)
            ...
            for _, data_loader in self.data_loaders.items():
                ...
                for i, batch in tqdm(enumerate(data_loader), total=len(data_loader)):
                    if self.args['test_speed']:
                        self.report_speed(model, batch, times=50)
                        continue
                    pred_output, dilate = model.forward(batch, training=False)
                    output = self.structure.representer.represent(batch, pred_output, dilate, is_output_polygon=self.args['polygon'])
                    ...
                    if visualize and self.structure.visualizer:
                        vis_image = self.structure.visualizer.visualize(batch, output, pred_output)
